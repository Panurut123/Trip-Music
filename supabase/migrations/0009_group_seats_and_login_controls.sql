-- Migration 0009: 40 logical student slots (Group A 01-20, Group B 01-20)
-- plus a hard login gate that works even before a slot has registered a profile.

CREATE TABLE IF NOT EXISTS public.seat_controls (
  room_id uuid NOT NULL REFERENCES public.rooms(id) ON DELETE CASCADE,
  seat_no integer NOT NULL CHECK (seat_no BETWEEN 1 AND 40),
  login_enabled boolean NOT NULL DEFAULT true,
  updated_at timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (room_id, seat_no)
);

ALTER TABLE public.seat_controls ENABLE ROW LEVEL SECURITY;

INSERT INTO public.seat_controls(room_id, seat_no, login_enabled)
SELECT r.id, n, true
FROM public.rooms r
CROSS JOIN generate_series(1, 40) n
ON CONFLICT (room_id, seat_no) DO NOTHING;

-- The profiles table was already widened by migration 0004; keep an explicit 1..40 constraint now.
ALTER TABLE public.profiles DROP CONSTRAINT IF EXISTS profiles_seat_no_check;
ALTER TABLE public.profiles ADD CONSTRAINT profiles_seat_no_check CHECK (seat_no BETWEEN 1 AND 40);
...
BEGIN
  IF p_seat_no < 1 OR p_seat_no > 40 THEN RAISE EXCEPTION 'invalid seat'; END IF;

  SELECT coalesce(sc.login_enabled, true) INTO v_login_enabled
  FROM (SELECT 1) seed
  LEFT JOIN public.seat_controls sc ON sc.room_id = p_room_id AND sc.seat_no = p_seat_no;
  ...
  RETURN jsonb_build_object(
    ...
    'nickname', CASE WHEN v_profile.id IS NULL THEN NULL ELSE v_profile.nickname END,
    'blocked', coalesce(v_profile.blocked, false),
    'login_enabled', coalesce(v_login_enabled, true)
  );
END;
$$;
...
BEGIN
  ...
  FROM (SELECT 1) seed
  LEFT JOIN public.seat_controls sc ON sc.room_id = p_room_id AND sc.seat_no = p_seat_no;
  IF NOT coalesce(v_login_enabled, true) THEN RAISE EXCEPTION 'login_disabled'; END IF;
  ...
  IF v_profile.id IS NOT NULL THEN
    IF v_profile.pin_hash IS NOT NULL THEN
      IF extensions.crypt(p_pin, v_profile.pin_hash) <> v_profile.pin_hash THEN RAISE EXCEPTION 'invalid_pin'; END IF;
    ELSIF v_profile.pin IS NOT NULL THEN
      IF v_profile.pin <> p_pin THEN RAISE EXCEPTION 'invalid_pin'; END IF;
      v_profile.pin_hash := extensions.crypt(p_pin, extensions.gen_salt('bf'));
    ELSE
      v_profile.pin_hash := extensions.crypt(p_pin, extensions.gen_salt('bf'));
    END IF;
    ...
    SET auth_user_id = auth.uid(), device_id = p_device_id, pin_hash = v_profile.pin_hash,
        pin = NULL, last_seen = now(), updated_at = now()
  ...
  VALUES (p_room_id, 'student', v_profile.id, 'student_joined',
          jsonb_build_object('seat_no', v_profile.seat_no, 'device_id', p_device_id));
...
BEGIN
    ...
    SELECT coalesce(sc.login_enabled, true) INTO v_login_enabled
    FROM (SELECT 1) seed
    LEFT JOIN public.seat_controls sc ON sc.room_id = p_room_id AND sc.seat_no = v_profile.seat_no;
    IF NOT coalesce(v_login_enabled, true) THEN RAISE EXCEPTION 'login_disabled'; END IF;
